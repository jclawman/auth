...
{ 
	A="$securekey"
        ...
        A+="$A2""$A3"
        echo "$A" | openssl sha256 | cut -c 1-28 | awk '{print $0".!."}'
        echo
	}
...
{ 
	B="$securekey"
        ...
        B+="$B2""$B3"
        echo "$B" | openssl sha256 | cut -c 1-28 | awk '{print $0".@."}'
        echo
	}
...
{ 
	C="$securekey"
        ...
        C+="$C2""$C3"
        echo "$C" | openssl sha256 | cut -c 1-28 | awk '{print $0".-."}'
        echo
	}
...
{ 
	D="$securekey"
        ...
        D+="$D2""$D3"
        echo "$D" | openssl sha256 | cut -c 1-28 | awk '{print $0".=."}'
        echo
	}
...
{ 
	E="$securekey"
        ...
        E+="$E2""$E3"
        echo "$E" | openssl sha256 | cut -c 1-28 | awk '{print $0".!."}'
        echo
	}
...
{ 
	F="$securekey"
        ...
        F+="$F2""$F3"
        echo "$F" | openssl sha256 | cut -c 1-12 | awk '{print $0".-."}'
        echo
	}
{ 
	G="$securekey"
        ...
        G+="$G2""$G3"
        echo "$G" | openssl sha256 | cut -c 1-12 | awk '{print $0".&."}'
        echo
	}
{ 
	H="$securekey"
        ...
        H+="$H2""$H3"
        echo "$H" | openssl sha256 | cut -c 1-12 | awk '{print $0".^."}'
        echo
	}
...
{ 
	I="$securekey"
        ...
        I+="$I2""$I3"
        echo "$I" | md5 | cut -c 1-12 | awk '{print $0".-."}'
        echo
	}
...
{ 
	J="$securekey"
        ...
        J+="$J2""$J3"
        echo "$J" | md5 | cut -c 1-12 | awk '{print $0"="}'
        echo
	}	
